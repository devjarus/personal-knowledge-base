    ...
    <div className="grid grid-cols-[1fr_220px] gap-8 max-w-5xl">
      <div className="min-w-0 space-y-4">
        <header>
          ...
          <div className="text-xs opacity-60 mt-1">{note.path}</div>
        </header>
        <NoteEditor initial={note} />
      </div>
      <aside
        className="text-xs space-y-3 border-l pl-4"
        style={{ borderColor: "var(--kb-border)" }}
      >
        <div className="font-semibold opacity-70 uppercase tracking-wide">metadata</div>
        {fmEntries.length === 0 && <div className="opacity-60">(no frontmatter)</div>}
        ...
        <div className="pt-2 border-t" style={{ borderColor: "var(--kb-border)" }}>
          <div className="opacity-60">size</div>
          <div className="font-mono">{note.size} bytes</div>
        </div>
        <div>
          <div className="opacity-60">mtime</div>
